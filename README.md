...
flowchart LR
    ...
    writer --> spool
    spool -->|ingest_pending_files()| service
    service -->|apply_spool_file()| storage
    storage -->|upsert агрегатов по UTC-часу| sqlite
    sqlite -.->|raw-времена не хранятся,\nтолько агрегаты| storage
```

Эта схема показывает границы между Firebird, `C++`-плагином и Python-сервисом.
Сначала trace-слой Firebird отдает событие завершения, затем `C++`-часть собирает агрегаты по UTC-часу и складывает их в spool.
Python-сервис подбирает готовые `JSONL`-снимки и переносит их в `SQLite`, не сохраняя полный список raw-длительностей.

## Детальная Схема C++ Trace-Плагина

```mermaid
flowchart TD
    needs["trace_needs()"]
    proc_cb["callback: procedure finish"]
    sql_cb["callback: SQL finish"]
    proc_finish["FirebirdTraceBridge::on_procedure_finish()"]
    sql_finish["FirebirdTraceBridge::on_sql_finish()"]
    trim["trim_identifier()"]
    classify["classify_sql_statement()"]
    record["UsageCollector::record_usage()"]
    filters["Фильтр БД\ninclude/exclude"]
    drop["Событие отброшено"]
    hour["format_usage_hour()\nUTC-час"]
    aggregate["Обновление in-memory aggregate\ncount / total / min / max / last_seen"]
    due["flush_if_due()?"]
    keep["Продолжаем копить\nв памяти"]
    detach["detach snapshot"]
    build["build_records()"]
    write["write_records()"]
    ok["JSONL записан\nв spool"]
    ...
    classify --> record
    record --> filters
    filters -->|подходит| hour
    filters -->|не подходит| drop
    hour --> aggregate
    aggregate --> due
    due -->|нет| keep
    due -->|да| detach
    ...
    build --> write
    write -->|успех| ok
    write -->|ошибка| restore
```

Эта схема показывает, почему trace-callback остается легким: внутри горячего пути происходит только нормализация события и быстрое обновление агрегата в памяти.
Сброс в spool вынесен в отдельный шаг `flush_if_due()`, поэтому запись на диск не выполняется на каждом вызове.
Если запись `JSONL` не удалась, snapshot возвращается обратно в память, и статистика не теряется.

## Детальная Схема Python-Сервиса

```mermaid
flowchart TD
    start["python3 -m proc_usage serve"]
    config["ServiceConfig\nspool_dir / sqlite_db_path /\npoll_interval_sec"]
    init["ProcUsageService.initialize()"]
    mkdir["Создать spool_dir\nесли каталога ещё нет"]
    dbinit["SQLiteUsageStorage.initialize()"]
    loop["serve_forever()"]
    ingest["ingest_pending_files()"]
    claim["SpoolIngestor.claim_files()"]
    rename["Переименование в .processing"]
    read["read_records()\nчтение JSONL-агрегатов"]
    apply["SQLiteUsageStorage.apply_spool_file()"]
    seen{"Файл уже\nобрабатывался?"}
    upsert["Upsert в SQLite:\ncount / total / min / max /\nlast_seen_at"]
    mark["mark_processed()"]
    sleep["wait(poll_interval_sec)"]
    stop{"stop_event\nвыставлен?"}
...
Эта схема показывает отдельный Python-процесс, который периодически просыпается, подбирает готовые `spool`-файлы и атомарно применяет их в `SQLite`.
Файлы сначала помечаются как `.processing`, чтобы их не взял второй процесс, а затем фиксируются в таблице обработанных spool-файлов.
Если конкретный файл уже был применён раньше, повторного добавления статистики не происходит.
